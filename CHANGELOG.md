...
### Fixed
- **Audio autoplay unlock**: music was silently failing to play due to browser autoplay policy. Added `audioUnlock()` (creates an ephemeral `AudioContext` to prime the audio subsystem) triggered by the first user gesture (`keydown`/`pointerdown`/`touchstart`) during the countdown, ensuring music starts reliably on all browsers.

### Added
- **Footer links**: both lobby (`index.html`) and game (`game.html`) footers now include a link to the GitHub repo and a PayPal donation link (☕).
...
[0.2.4]: https://github.com/davic80/tetris-battle/compare/v0.2.3...v0.2.4
